...
public:
   ...
   int NI, NO, capacity;
   int itime = 0, T = 1, lost = 0, ni, no, high = 0, sum = 0,sum2=0;   //고정값들, 초기에는 손실 0
   float average = 0, fullness=0,loss_rate=0;
   int NofPackets[120];
   ...
   int que(ofstream& xx) {
      ...
      cout << " Queue fullness: " << fullness << " Packet loss rate: " << loss_rate << endl;   //실험값 cout
      xx << NO << " " << (float)NO / NI <<" "<< fullness << " "<<loss_rate<<endl ;
      return 0;
   }
};
